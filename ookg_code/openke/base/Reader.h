...
void importTestFiles() {
    ...
    fin = fopen((inPath + "entity_ind.txt").c_str(), "r");
    for (INT i = 0; i < len_ent; i++) {
        //std::cout<<i<<std::endl;
        tmp = fscanf(fin, "%ld", &e2e[i]);
    ...
    FILE* f_kb1s,  * f_kb1o, * f_kb2, * f_kb3;
    if (train_file == "")
        f_kb2 = fopen((inPath + "train2id.txt").c_str(), "r");
    else
        f_kb2 = fopen(train_file.c_str(), "r");
    if (test_file == "")
        f_kb1s = fopen((inPath + "testsub.txt").c_str(), "r");
    else
        f_kb1s = fopen(test_file.c_str(), "r");
    if (test_file == "")
        f_kb1o = fopen((inPath + "testobj.txt").c_str(), "r");
    else
        f_kb1o = fopen(test_file.c_str(), "r");	
    if (valid_file == "")
        f_kb3 = fopen((inPath + "valid2id.txt").c_str(), "r");
    else
        f_kb3 = fopen(valid_file.c_str(), "r");
    tmp = fscanf(f_kb1s, "%ld", &testTotals);
    tmp = fscanf(f_kb1o, "%ld", &testTotalo);
	testTotal = testTotals + testTotalo;
    tmp = fscanf(f_kb2, "%ld", &trainTotal);
    tmp = fscanf(f_kb3, "%ld", &validTotal);	
    tripleTotal = testTotals + testTotalo + trainTotal + validTotal;
    testList = (Triple *)calloc(testTotal, sizeof(Triple));	
    testLists = (Triple *)calloc(testTotals, sizeof(Triple));
    testListo = (Triple *)calloc(testTotalo, sizeof(Triple));	
    validList = (Triple *)calloc(validTotal, sizeof(Triple));
    tripleList = (Triple *)calloc(tripleTotal, sizeof(Triple));
    for (INT i = 0; i < testTotals; i++) {
        tmp = fscanf(f_kb1s, "%ld", &testLists[i].h);
        tmp = fscanf(f_kb1s, "%ld", &testLists[i].t);
        tmp = fscanf(f_kb1s, "%ld", &testLists[i].r);
        tripleList[i] = testLists[i];
		testList[i] = testLists[i];
		
    }
	
	for (INT i = 0; i < testTotalo; i++) {
        tmp = fscanf(f_kb1o, "%ld", &testListo[i].h);
        tmp = fscanf(f_kb1o, "%ld", &testListo[i].t);
        tmp = fscanf(f_kb1o, "%ld", &testListo[i].r);
        tripleList[i+ testTotals] = testListo[i];
		testList[i+ testTotals] = testListo[i];		
    }
    for (INT i = 0; i < trainTotal; i++) {

...
void getHeadBatch(INT *ph, INT *pt, INT *pr) {
    for (INT i = 0; i < entityTotal; i++) {
        ph[i] = i;
        pt[i] = testListo[lastHead].t;
        pr[i] = testListo[lastHead].r;
    }
    lastHead++;
}
...
void getTailBatch(INT *ph, INT *pt, INT *pr) {
    for (INT i = 0; i < entityTotal; i++) {
        ph[i] = testLists[lastTail].h;
        pt[i] = i;
        pr[i] = testLists[lastTail].r;
    }
    lastTail++;
}
...
void testHead(REAL *con, INT lastHead, bool type_constrain = false) {
    INT h = testListo[lastHead].h;
    INT t = testListo[lastHead].t;
    INT r = testListo[lastHead].r;
    INT lef, rig;
    if (type_constrain) {
        lef = head_lef[r];
        rig = head_rig[r];
...
void testTail(REAL *con, INT lastTail, bool type_constrain = false) {
    INT h = testLists[lastTail].h;
    INT t = testLists[lastTail].t;
    INT r = testLists[lastTail].r;
    INT lef, rig;
    if (type_constrain) {
        lef = tail_lef[r];
        rig = tail_rig[r];
...
void test_link_prediction(bool type_constrain = false) {
    l_rank /= testTotalo;
    r_rank /= testTotals;
    l_reci_rank /= testTotalo;
    r_reci_rank /= testTotals;
    ...
    // with filter
    l_filter_rank /= testTotalo;
    r_filter_rank /= testTotals;
    l_filter_reci_rank /= testTotalo;
    r_filter_reci_rank /= testTotals;
 
    l_filter_tot /= testTotalo;
    l3_filter_tot /= testTotalo;
    l1_filter_tot /= testTotalo;
 
    r_filter_tot /= testTotals;
    r3_filter_tot /= testTotals;
    r1_filter_tot /= testTotals;
    ...
    mrr = (l_filter_reci_rank * testTotalo +r_filter_reci_rank * testTotals) / (testTotalo + testTotals);
    mr = (l_filter_rank * testTotalo +r_filter_rank * testTotals) / (testTotalo + testTotals);
    hit10 = (l_filter_tot * testTotalo +r_filter_tot * testTotals) / (testTotalo + testTotals);
    hit3 = (l3_filter_tot * testTotalo +r3_filter_tot * testTotals) / (testTotalo + testTotals);
    hit1 = (l1_filter_tot * testTotalo +r1_filter_tot * testTotals) / (testTotalo + testTotals);

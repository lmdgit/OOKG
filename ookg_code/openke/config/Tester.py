...
class Tester(object):
    ...
    def run_link_prediction(self, type_constrain = False):
        self.lib.initTest()
        self.data_loader.set_sampling_mode('olink')
        print("test on the testobj.txt")
        if type_constrain:
            type_constrain = 1
        else:
            type_constrain = 0
        training_range = tqdm(self.data_loader)
        for index, [data_head] in enumerate(training_range):
            score = self.test_one_step(data_head)
            self.lib.testHead(score.__array_interface__["data"][0], index, type_constrain)
        
        self.data_loader.set_sampling_mode('slink')
        print("test on the testsub.txt")		
        training_range = tqdm(self.data_loader)        
        for index, [data_tail] in enumerate(training_range):
